...
export function getCachedResults(urlString, deviceType, type, suffix = '', model = '') {
  ...
  // Special handling for report and rules summaries
  if (type === 'report' || type === 'rules') {
    // First check the new reports directory for summary files
    const newPath = getCachePath(urlString, deviceType, type, suffix, true, model);
    if (fs.existsSync(newPath)) {
      const content = fs.readFileSync(newPath, { encoding: 'utf8' });
      return { content }; // Wrap in object to match expected format
    }
    
    // For backward compatibility, check the old cache location
    const cleanUrl = urlString.replace('https://', '').replace(/[^A-Za-z0-9-]/g, '-').replace(/\//g, '--').replace(/(^-+|-+$)/, '');
    const oldPath = `${OUTPUT_DIR}/${cleanUrl}.${deviceType}.${type}${suffix ? `.${suffix}` : ''}${modelSuffix(model)}.summary.md`;
    if (fs.existsSync(oldPath)) {
      const content = fs.readFileSync(oldPath, { encoding: 'utf8' });
      
      // Optionally migrate the file to the new location
      console.log(`Migrating report from ${oldPath} to ${newPath}`);
    ...
    // Finally check for JSON format (for non-summary report data)
    const jsonPath = `${getFilePrefix(urlString, deviceType, type)}${suffix ? `.${suffix}` : ''}${modelSuffix(model)}.json`;
...
export function getCachePath(urlString, deviceType, type, suffix = '', isSummary = false, model = '') {
  ...
  // Use reports directory for report summaries and rules summaries
  const isReportOutput = (type === 'report' || type === 'rules') && isSummary;
  const baseDir = isReportOutput ? REPORTS_DIR : OUTPUT_DIR;
  
  // For reports, create a cleaner file structure
  if (isReportOutput) {
    const cleanUrl = urlString.replace('https://', '').replace(/[^A-Za-z0-9-]/g, '-').replace(/\//g, '--').replace(/(^-+|-+$)/, '');
    const basePrefix = `${baseDir}/${cleanUrl}.${deviceType}.${type}`;
    return buildPath(basePrefix, suffix, model, 'summary.md');
  }
  ...
  // Handle summary files (non-report summaries stay in cache)
  if (isSummary) {
...
export function cacheResults(urlString, deviceType, type, results, suffix = '', model = '') {
  ...
  // Ensure reports directory exists if we're saving a report summary
  const isSummary = typeof results === 'string' && (type === 'report' || type === 'rules');
  if (isSummary) {
    ensureDir(REPORTS_DIR);
  }
